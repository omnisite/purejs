(function MakeApp(sys, dispatch, cont, bind) {

	sys.dispatch = dispatch;
	sys.nextTick = dispatch(unit)();
	sys.makeCont = cont(sys.nextTick.enqueue);
	sys.makeBind = bind(sys.nextTick.enqueue);
	return (self.sys = sys);
	...
	(function MakeSys() {
		...
		(function MakeFlatten() {
			...
			this.prototype.flatmap = function(f, k) {
				return this.bind(bind(f, []), k);
			};
	...
	(function MakeCont() {
		...
		(function wrap(items) {
			return function(enqueue) {
				items.push(enqueue);
				return items.apply(0);
			}
		}),
		(function make(ext, wrap, combine, enqueue) {
	...
	(function MakeBind() {
		...
		(function wrap(make, bind, cont) {
			return function(enqueue) {
				return make.call(Array, bind, cont, enqueue);
			}
		}),
		(function make(bind, cont, enqueue) {
			this.prototype.bind = function(f, k, r) {
				return enqueue(bind(f, cont, enqueue)(this, cont([], k)));
			};
			return this;
		}),
		(function bind(f, c, e) {
			return function next(x, k) {
				var r = [];
				return function() {
					if (r.length == x.length) {
						k(r);
						return true;
					}else if (x[r.length] instanceof Array) {
						//x[r.length].bind(f, k, r[r.push([])-1]);
						e(next(x[r.length], c(r[r.push([])-1], k)));
					}else {
						r.push(f(x[r.length]));
					}
				};
