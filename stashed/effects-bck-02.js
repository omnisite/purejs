        ...
        // === Environment Setup === //
            ...
            (function Setup(env, defs, sys) {
                ...
                return eff.runDefs(eff, defs(unit)).chain(function(result) {
                    ...
                    sys.eff('sys.loader.component').run('modules/application/application').create('home').run(unit);
                });
            }),
                                        ...
                                        function ComponentCont(mv, mf) {
                                            ...
                                            mf: function mf(tag) {
                                                return function $_pure(k) {
                                                    var cont = sys().find(tag.getAttribute('data-ref')).get('cont');
                                                    k(cont && cont instanceof Function ? cont() : cont);
                                                };
                                            }
                                        }, {
                                            of: function(loc) {
                            ...
                            (function makeEnqueue(makeWrap, makeOf, makeBind, makeSetRes, makeResult) {
                                ...
                                (function makeWrap(of, detect) {
                                    return function enqueue(def, run) {
                                        ...
                                        if (!cont) {
                                            cont = store.set('cont', $const(of(def, run, {}).cont()));
                                            cont.name = def.name;
                                            cont.ref  = store.uid();
                                        }else {
                                            debugger;
