...
// ========  ======== STAGE MAKE ========  ======== //
    ...
    (function Factory() {
        ...
        (function BaseModel(dataStore, sysModel, sysControl, dispatcher) {
            ...
            return (self.sys = dataStore(sysModel(), dispatcher)(sysControl));
        }),

        (function RootDataStore(model, dispatcher) {

            var Store   = model('store');
            var Node    = model('node');
            Node.prototype.$store = model('factory.deriving')('_store');
            Store.prototype.store = model('func.store.store');
            var Storage = model('storage'); Storage.prototype.node = Store; Storage.prototype._store = Store.of('root');

            var DB      = Store.prototype.storage = Node.prototype.storage = Storage.of('db');
            var Root    = Storage.prototype.root  = Node.of('root'); Root.set('storage', DB);

            Store.prototype.root = Node.prototype.root = Root.ext('target')('get');

            Root.child('types');
            Root.child('sys').set('cntrl', {});
            Root.child('sys').set('model', model);
            var utils = Root.child('utils').ext('parse')(model('func.base'));

            var scheduler = Root.child('scheduler');
            scheduler.set('dispatcher', dispatcher);

            var Queue = model('queue'), Main = Queue.of('queue');
            Queue.prototype.schedule = scheduler.set('nextTick', dispatcher(unit)()).enqueue;
            Queue.prototype.enqueue  = Main.enqueue.bind(Main);

            model('factory')(unit).ext(Queue.prototype, model('func.push'));
            Root.set('queue', Main);

            return utils.get('tuple')(model)(Root);
        }),
            ...
            return {
                ctor: {
                    ...
                    queue: function Queue(opts) {
                        ...
                        this._handlers = this.set('handlers', []);
                    },
                ...
                func: {
                    base: [
                        ...
                        (function $comprehension(map, filter) {
                    ...
                    node: [
                        ...
                        (function type() {
                            return sys.type(this.constructor.name);
                        }),
                        (function cached(key) {
                        ...
                        (function get(key) {
                            return typeof key == 'undefined' ? this : this._store.get(key);
                        }),
                        (function set(key, value) {
                            return this._store.set(key, value);
                        }),
                        (function acc(key, value) {
                            return this._store.acc(key, value);
                        }),
                        (function values(recur) {
                    ...
                    push: [
                        (function push(key, value, asArray) {
                            ...
                            if ((index = this._map[key]) >= 0) {
                                var prop = this._val[index], idx = 0;
                                if (prop instanceof Array) {
                                    if (prop.length && (idx = prop.indexOf(value)) >= 0) return idx;
                                    pos = prop.push(value);
                                    this.emit('change', this._ids[index], 'push', prop);
                                    return pos;
                                }else {
                                    this.emit('change', this._ids[index], 'set', value);
                                    return (this._val[index] = value) ? (index + 1) : (index + 1);
                                }
                            }else {
                                this.emit('create', key, 'push', value);
                                return (this._map[this._ids[(this._ids.push(key) - 1)]] = this._val.push(!asArray || value instanceof Array ? value : [ value ]) - 1) + 1;
                            }
                        }),
                        (function shift(key) {
                            var index, parts, prop, value, node = this;
                            if ((index = node._map[key]) >= 0) {
                                prop = node._val[index];
                                if (prop instanceof Array && prop.length) {
                                    value = prop.shift();
                                    node.emit('change', key, 'remove', value);
                                    return value;
                                }
                            }
                        }),
                        (function splice(key, idx, num) {
                            ...
                            if ((index = node._map[key]) >= 0) {
                                ...
                                num || (num = 1);
                                if (prop instanceof Array && prop.length) {
                                    value = prop.splice(idx, num || 1);
                                    node.emit('change', key, 'remove', num == 1 ? value[0] : value);
                                    return value;
                                }
                            }
                        })
                    ...
                    queue: [
                        (function enqueue(item) {
                            if (this.push('queue', item, true) == 1) {
                                this.schedule(this);
                            }
                            return this;
                        }),
                    ...
                    signal: [
                        ...
                        (function resolve(value) {
                            return this.handle(value, 'resolved');
                        }),
                        (function cancel(message) {
                            return this.handle(message, 'cancelled');
                        }),
                        (function error(e) {
                            return this.handle(e, 'error');
                        })
                    ],
                    scheduler: {
                        async: [
                    ...
                    stream: [
                        ...
                        (function elem(/* type, elem, name */) {
                            ...
                            if (!id || !(node = this.elements.get(id)) || !(node = node.get(name)) || !(node.get(type))) {
                                ...
                                if (!name) debugger;
                                node = this.elements.child({ name: id, id: id }).child(name);
                ...
                factory: {
                    ...
                    object: function(path, ext) {
                        var model = this.model || this;
                        var items = model.find(path);
                        var objct = model.factory.ext(ext || {}, items);
                        return objct;
                    },
                    ...
                    deriving: function(prop) {
                        return function(fn) {
                            return this[prop] && this[prop][fn]
                                ? Function.prototype.bind.call(this[prop][fn], this[prop])
                                    : unit;
                        }
                    },
                    inherit: function(type, parent) {
                        ...
                        if (!klass.of) klass.of = type.of || this.of;
                        proto.constructor = klass; proto.ctor = type.ctor;
                        klass.prototype = proto;
                        if (!klass.prototype.is) klass.prototype.is = model.find('func', 'klass', 'is');
                        if (!klass.prototype.id) klass.prototype.id = model.find('func', 'klass', 'makeID')();
                        return klass;
                    },
                    ...
                    type: function(name) {
                        var model = this.model, type = model.find('types', name) || { name: name };
                        if (!type.ctor)   type.ctor  = model.find('ctor', name);
                        if (!type.ext)    type.ext   = model.find('func', name);
                        if (type.ext)     type.ext   = this.unwrap(type.ext);
                        if (this.model.attrs[name]) type.attrs = this.model.find('attrs', name);
                        if (!type.klass)  type.klass = model.find('func', 'core', 'named')(name);
                        if (!type.super) {
                            type.super = type.parent || model.find('model', 'parents', name);
                            if (type.super) {
                                type.parent = model.find('types', 'ctor', type.super)
                                    || this.make(model, type.super);
                            }
                        }
                        if (!type.deriving) {
                            type.deriving = (model.find('model', 'deriving', name) || []).map(function(deriv) {
        ...
        (function MakeSysControl(model) {
            return function(root) {
                var sys   = root.sys = {};
                sys.get   = root.$store('get');
                sys.model = model;
                return sys;
            }
        }),
        ...
        (function() {
            return {
                ...
                ext: function() {
                    ...
                    var maybeLifter = maybe(function(lift2) {
                    ...
                    var maybeEventBinder = maybe(function(dispatcher) {
                    ...
                    return [
                        { name: 'mainDispatcher', fn: maybeEventBinder(sys.get('queue').enqueue) },
                        ...
                        { name: 'DOMeventHandler', fn: DOMeventHandler }
                    ];
                },
                attrs: [
                    (function init(elem) {
                        var func = this.prototype;
                        var bind = func.maybeEventDOM(elem);
                        return this.of(func.mainDispatcher(bind));
                    })
                ],
                init: function() {
    ...
    (function LinkArray() {
        ...
        (function InitContext() {
            return this.model('func')('array', {
                async: this.model('func.scheduler.async').append(this.get('scheduler').get('nextTick.enqueue')).apply(),
...
// ========  ======== FINISH  ========  ======== //
    ...
    (function RunApp() {
        ...
        (function RunAppStream() {
            ...
            Stream.elem = Stream.prototype.elem; Stream.prototype.elements = Root.child('elements');
